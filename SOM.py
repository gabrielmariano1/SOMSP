...
class SOM:
	...
	def __init__(self, input_shape, map_size_n, num_expected_iterations, session):
		...
		for r in list(range(self.n)):
			for c in list(range(self.n)):
		...
		for i in list(range(self.n*self.n)):
			for j in list(range(self.n*self.n)):
	...
	def initialize_graph(self):
		...
		for i in list(range(len(self.input_shape))):
...
with tf.device("CPU:0"):
	...
	values_list = []
	coodgeo = []
	with open("DEINFO_CT_CULTURA_2013.csv") as csvfile:
		reader = csv.DictReader(csvfile)
		reader = csv.reader(csvfile, delimiter=';')
		for row in reader:
			try:
				coodgeo.append([(abs(float(row[2].replace(',','.')))-23)**coef_dist,(abs(float(row[1].replace(',','.')))-46)**coef_dist])
			except ValueError:
				continue

	with open("escolasr34abril2017.csv") as csvfile:
		reader = csv.DictReader(csvfile)
		reader = csv.reader(csvfile, delimiter=';')
		for row in reader:
			try:
				coodgeo.append([(abs(float(row[49])/1000000.)-23)**coef_dist,(abs(float(row[50])/1000000.)-46)**coef_dist])
			except ValueError:
				continue
				
	num_training = len(coodgeo)*3
	s = SOM( (2,), map_size, num_training, sess )
	...
	coodgeo = np.array(coodgeo)
	for i in list(range(num_training)):
		rnd_ind = np.random.randint(0, len(coodgeo))
		s.train(coodgeo[rnd_ind,:])
		...
		for row in reader:
			try:
				if(row[0][0]!=' '):
					splitt = row[1].split(",")
					if len(splitt)>1:
						splitt[0] = float(splitt[0])
						splitt[1] = float(splitt[1])
						if not((splitt[0] < -24 and splitt[0] > -22) or (splitt[1] < -47 and splitt[1] > -45)):
							transito.append([(abs(splitt[0])-23)**coef_dist,(abs(splitt[1])-46)**coef_dist])
			except ValueError:
				continue
        
	classificacao = s.get_batch_winner(transito)
	
	plt.subplot(211)
	plt.hist(classificacao[0],np.arange(map_size*map_size+1),(1,map_size*map_size+1))
	plt.xticks(range(1,map_size*map_size+1))
	#plt.show()
	
	labels = np.arange(map_size*map_size+1)
	plt.subplot(212)
	m = 0
	for i in s.get_weights():
		print("{location: new google.maps.LatLng(	",-1*(i[0]**(1./coef_dist)+23)	,",",	-1*(i[1]**(1./coef_dist)+46),"	), weight: 	2049	},")
		#print(-1*(i[0]**(1./coef_dist)+23),",",-1*(i[1]**(1./coef_dist)+46),str(k),1-i[1],1-i[0])
		plt.plot(1-i[1]**(1./coef_dist),1-i[0]**(1./coef_dist),marker='o',c=[i[0],i[1],0])
		m+=1
		plt.text(1-i[1]**(1./coef_dist),1-i[0]**(1./coef_dist), labels[m], ha='center', va='center',
             bbox=dict(facecolor='white', alpha=0.5, lw=0))

		
	#plt.imshow( np.reshape(s.get_weights(), [map_size, map_size, 2]) )
